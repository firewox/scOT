...
# linux版本
data_r=pd.read_csv("/mnt/usb/code/lyutian/git_repositories/SCAD/data/split_norm/"+str(DRUG)+"/allGenes"+"/Source_expr_resp_z."+str(DRUG)+str(args.geneset)+".tsv", sep='\t', index_col=0, decimal='.') # source_data_path = args.bulk_data
# windows版本
...
# linux版本
data_t=pd.read_csv("/mnt/usb/code/lyutian/git_repositories/SCAD/data/split_norm/"+str(DRUG)+"/allGenes"+"/Target_expr_resp_z."+str(DRUG)+str(args.geneset)+".tsv", sep='\t', index_col=0, decimal='.')
# windows版本
...
# 创建新的列名列表
data_bulk_1 = data_bulk.copy()
data_sc_1 = data_sc.copy()
...
if data_bulk.shape[1] > data_sc.shape[1]:
    ...
    new_columns = [f'a_{i+data_sc.shape[1]}' for i in range(l)]
    zeros_data = pd.DataFrame(0, columns=new_columns, index=data_sc.index)
    data_sc_1 = pd.concat([data_sc_1, zeros_data], axis=1)
elif data_bulk.shape[1] < data_sc.shape[1]:
    l = data_sc.shape[1] - data_bulk.shape[1]
    new_columns = [f'a_{i+data_bulk.shape[1]}' for i in range(l)]
    zeros_data = pd.DataFrame(0, columns=new_columns, index=data_bulk.index)
    data_bulk_1 = pd.concat([data_bulk_1, zeros_data], axis=1)
    #for i in range(l):
...
adata_cm_1 = data_bulk_adata_1.concatenate(data_sc_adata_1, join='inner', batch_key='domain_id')
print(f'adata_cm_1==={adata_cm_1}') #n_obs × n_vars = 895 × 10610
print(f'data_bulk_adata==={data_bulk_adata}') #n_obs × n_vars = 895 × 10610
print(f'data_sc_adata==={data_sc_adata}') #n_obs × n_vars = 895 × 10610
...
for i in range(args.n_replicates):
    ...
    adata = up.Run3(adatas=[data_bulk_adata, data_sc_adata], adata_cm=adata_cm_1, num_workers=4,
                    ...
                    unshared_encoder=bool(args.unshared_encoder),) #TODO,new feature
###########################################################2.模型训练，END
